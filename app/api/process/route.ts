...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(landmarks) || landmarks.length === 0) {
      return NextResponse.json(
        { status: "error", error: "Invalid landmarks format or empty landmarks array" }, 
        { status: 400 }
      );
    }
    ...
    const modelApiUrl = process.env.MODEL_API_URL || "http://localhost:5000/process";
    ...
    if (!modelResponse.ok) {
      throw new Error(`Model API responded with status: ${modelResponse.status}`);
    }
